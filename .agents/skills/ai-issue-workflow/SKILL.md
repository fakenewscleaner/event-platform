...
query($endCursor: String, $issueNumber: Int!) {
  repository(owner: "fakenewscleaner", name: "event-platform") {
...
用 `gh api graphql -f query='...'` 執行以上查詢/mutation 即可，不需要額外套件；執行前確認 `gh` 已安裝且已登入有權限存取此 repo 的帳號。

### 分頁與查詢參數

- 查詢 1 使用 `gh api graphql --paginate -f query='...'`，由 `--paginate` 配合 `$endCursor` 與外層 `issues.pageInfo` 讀完所有 open issue；必須處理每一頁回傳資料。
- 巢狀的 `issueFieldValues` 不會隨外層 issues 自動完成分頁。若其 `hasNextPage` 是 true 且尚未找到 `wait_for`，針對該 issue 另查 `issue(number: $issueNumber) { issueFieldValues(first: 100, after: $endCursor) { ... } }`，保留同樣的 nodes 與 pageInfo 選取，逐頁查到欄位或最後一頁。
- 查詢 2 使用 `gh api graphql --paginate -F issueNumber=<實際 issue 編號> -f query='...'`，逐頁讀取 repo 的欄位，依名稱找出 `wait_for`。不要使用範例編號或硬編欄位、選項 ID。
- 查詢失敗或分頁未完成時，不可把未取得的資料視為沒有待辦 issue。
...
其他已知選項：`AI_plan`、`AI_explain`、`AI_coding`、`Humain_review`、`to_merge`。

## 執行流程

1. 檢查專案指引與目前 Git 狀態，包括現有程式碼、分支、是否已有 commit 及未提交修改；依執行當下的狀態工作，保留使用者既有修改。
2. 用上面的查詢 1 完整分頁掃描 open issue，找出 `wait_for` 恰為 `AI_plan` / `AI_explain` / `AI_coding` 的 issue。
3. 對每個符合的 issue：
   - 用 `gh issue view <number> --repo fakenewscleaner/event-platform --comments` 讀完整內文與所有留言。
   - 讀專案中相關的檔案（如果 repo 裡已經有程式碼的話）。
   - 由目前 agent 執行對應動作；環境支援且允許委派時，可交給 subagent。若多個 issue 涉及相同檔案，應依序處理或隔離工作目錄，避免互相覆蓋：
     - **`AI_plan`**：針對 issue 內容回覆接下來的開發計劃，盡量簡潔，必要時可在留言附示意程式碼。只回覆計畫，不修改專案檔案、不 commit。
     - **`AI_explain`**：針對討論內容回答問題，只回覆留言。
     - **`AI_coding`**：表示計劃已確認，開始實際開發，完成後發一個 commit。開發過程中如果有任何問題沒有釐清，就暫停、不要硬做，回覆該 issue 說明問題卡在哪裡，然後停下來等人類回應（不要把 wait_for 改成 Humain_review）。
   - 留言用 `gh issue comment <number> --repo fakenewscleaner/event-platform --body-file <暫存檔>`（內容先寫進暫存檔再用 `--body-file`，避免 shell 轉義問題）。
4. 對於已經完整完成 `AI_plan` 回覆或 `AI_explain` 回答、或 `AI_coding` 開發並成功 commit 的 issue，用上面的 mutation 3 把該 issue 的 `wait_for` 改成 `Humain_review`。
